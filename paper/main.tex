...
\begin{center}
    ...
    \begin{tabular}{ccccc}
        ...
        {\footnotesize Mew (Original)} && {\footnotesize Mew (Encrypted transit image)} && {\footnotesize Mew (1000-epoch Decrypter reconstruction)}
    \end{tabular}
\end{center}
...
\begin{equation}
x_0 = (r_0, g_0, b_0, a_0) \in \{0,\dots,255\}^{H \times W \times 3} \times [0,1]^{H \times W \times 1}.
\end{equation}
...
\end{align}
In repository code and CLI flags these stages are still named `encoder' and `decoder' for backward compatibility, but in the paper we refer to $f_{\theta_e}$ as the \emph{Encrypter} and $f_{\theta_d}$ as the \emph{Decrypter}. Both are instantiated as the same compact U-Net-like CNN with \ModelParamCount\ trainable parameters, built from three downsampling blocks, a bottleneck, and three decoder blocks with skip connections following the U-Net design pattern \citep{ronneberger2015unet}. The network predicts three separate $256$-way categorical distributions, one for each RGB channel, and the output alpha channel is copied from the input image.

Training uses a per-channel categorical objective on RGB:
\begin{align}
\mathcal{L}_{\text{enc}}(\theta_e) &= \frac{1}{3N}\sum_{i=1}^{N} \sum_{c \in \{R,G,B\}} \operatorname{CE}\!\left(f_{\theta_e}^{(c)}(x_0^{(i)}), z_c^{(i)} \right), \\
\mathcal{L}_{\text{dec}}(\theta_d) &= \frac{1}{3N}\sum_{i=1}^{N} \sum_{c \in \{R,G,B\}} \operatorname{CE}\!\left(f_{\theta_d}^{(c)}(z^{(i)}), x_{0,c}^{(i)} \right).
\end{align}
Here $\operatorname{CE}$ denotes cross-entropy over the $256$ discrete RGB values for one channel. The repository still reports dataset mean absolute error (MAE) after converting predicted RGB logits back to discrete $8$-bit values and composing them with the preserved alpha channel.
...
\begin{itemize}
    ...
    \item Objective: per-channel cross-entropy on discrete RGB values, with alpha preserved exactly.
    \item Training logs used here: continued Encrypter and Decrypter runs up to 1000 epochs from the repository's `models/paper\_eval` directory.
\end{itemize}
...
Both endpoint networks reach useful error levels quickly, although the discrete-RGB objective is noisier than the earlier continuous-regression baseline. Figure~\ref{fig:training-curves} shows steady overall improvement together with the expected epoch-to-epoch oscillations from categorical training on a small dataset. Extending training beyond the initial short run helped materially. The Encrypter best MAE improved from about $0.00628$ at 60 epochs to $0.000080$ by 200 epochs and $0.0000669$ by 1000 epochs. The Decrypter improved even more meaningfully, from about $0.0282$ in the earlier short run to $0.0223$ by 200 epochs and $0.00395$ by 1000 epochs.
...
\begin{figure}[t]
    ...
    \caption{Left: original and encrypted RGB intensity distributions. Right: error reduction relative to the direct baseline. The current Decrypter improves over the baseline substantially, but the result is still learned reconstruction rather than evidence of full reversibility.}
...
\begin{enumerate}
    ...
    \item Investigate continuous-color extensions beyond the current discrete RGB lattice, likely with richer output heads and larger, more diverse training datasets.
\end{enumerate}
...
\begin{center}
    ...
    \begin{tabular}{ccc}
        ...
        \includegraphics[width=0.13\linewidth]{../pokemon/images/roselia.png} &
        \includegraphics[width=0.13\linewidth]{../pokemon/images/vivillon.png} &
        \includegraphics[width=0.13\linewidth]{../pokemon/images/claydol.png} \\
        {\footnotesize Original} &
        {\footnotesize Original} &
        {\footnotesize Original} \\
        \includegraphics[width=0.13\linewidth]{../pokemon_restored/roselia.png} &
        \includegraphics[width=0.13\linewidth]{../pokemon_restored/vivillon.png} &
        \includegraphics[width=0.13\linewidth]{../pokemon_restored/claydol.png} \\
        {\footnotesize 1000-epoch Decrypter output} &
        {\footnotesize 1000-epoch Decrypter output} &
        {\footnotesize 1000-epoch Decrypter output}
    \end{tabular}
\end{center}
