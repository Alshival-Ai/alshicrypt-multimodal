...
\begin{center}
    ...
    \begin{tabular}{ccccc}
        ...
        \includegraphics[width=0.13\linewidth]{../pokemon/images/mew.png} \\
        {\footnotesize Mew (Original)} && {\footnotesize Mew (Encrypted transit image)} && {\footnotesize Mew (Decrypted target)}
    \end{tabular}
\end{center}
...
\begin{itemize}
    ...
    \item Training logs used here: 20-epoch Encrypter and Decrypter runs from the repository's quick-eval model directory.
\end{itemize}
...
Figure~\ref{fig:reconstruction-grid} illustrates the current visual behavior. The encrypted targets are visually noisy and semantically obscured. The Encrypter should therefore be judged by how closely it matches that distorted transit target, not by similarity to the original image. The learned Encrypter captures the silhouette well but does not reproduce the target's multicolored endpoint distribution exactly. The learned Decrypter reconstructs shape reliably, but the recovered images remain muted and often collapse toward silhouette-first solutions.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{reconstruction_grid.png}
    \caption{Qualitative examples from the current repository checkpoints: \EncoderExampleA, \EncoderExampleB, \EncoderExampleC, and \EncoderExampleD. Left-to-right within each example, the Encrypter output is supposed to resemble the distorted encrypted target rather than the original image. The Decrypter recovers recognizable geometry, but color fidelity remains limited, which is consistent with the severe endpoint information loss predicted by the theory.}
